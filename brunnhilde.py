...
def run_clamav(args, source_dir):
    ...
    if os.path.getsize(virus_log) > 40:  # check to see if clamscan actually ran
        if "Infected files: 0" not in open(virus_log).read():
            print(
                "\nWARNING: Infected file(s) found in %s. See %s for details."
                % (source_dir, virus_log)
            )
        else:
...
def get_stats(
        ...
        for root, dirs, files in os.walk(unicode(source_dir, "utf-8")):
            for f in files:
                ...
                try:
                    ...
                    print(
                        "\nOSError: %s. File size of this file not included in Brunnhilde HTML report statistics."
                        % (e)
                    )
    ...
    html.write('\n<style type="text/css">{}</style>'.format(BOOTSTRAP_CSS))
    html.write("\n</head>")
    ...
    html.write('\n<a class="nav-item nav-link text-light" href="#Provenance">Provenance</a>')
    html.write('\n<a class="nav-item nav-link text-light" href="#Stats">Statistics</a>')
    if not (args.noclam or sys.platform.startswith("win")):
        html.write('\n<a class="nav-item nav-link text-light" href="#Virus report">Virus report</a>')
    html.write('\n<a class="nav-item nav-link text-light" href="#File formats">File formats</a>')
    html.write(
        '\n<a class="nav-item nav-link text-light" href="#File format versions">Versions</a>'
    )
    html.write('\n<a class="nav-item nav-link text-light" href="#MIME types">MIME types</a>')
    html.write(
        '\n<a class="nav-item nav-link text-light" href="#Last modified dates by year">Dates</a>'
    )
    html.write('\n<a class="nav-item nav-link text-light" href="#Unidentified">Unidentified</a>')
    if args.warnings:
        html.write('\n<a class="nav-item nav-link text-light" href="#Warnings">Warnings</a>')
    html.write('\n<a class="nav-item nav-link text-light" href="#Errors">Errors</a>')
    if use_hash:
        html.write('\n<a class="nav-item nav-link text-light" href="#Duplicates">Duplicates</a>')
    if args.bulkextractor:
        html.write('\n<a class="nav-item nav-link text-light" href="#SSNs">SSNs</a>')
    html.write("\n</div>")
    html.write("\n</div>")
    html.write("\n</nav>")
    # content
    ...
    if use_hash:
        ...
        if duplicate_copies:
            html.write(' (of {} distinct files) <a href="#Duplicates">(see list)</a></p>'.format(distinct_dupes))
        else:
    ...
    # virus report
    if not (args.noclam or sys.platform.startswith("win")):
        html.write('\n<a name="Virus report" style="padding-top: 40px;"></a>')
        html.write('\n<div class="container-fluid" style="margin-bottom: 40px;">')
        html.write('\n<div class="card">')
        html.write('\n<h2 class="card-header">Virus report</h2>')
        html.write('\n<div class="card-body">')
...
def process_content(
    ...
    if args.bulkextractor:
        ...
        write_html_report_section("SSNs", os.path.join(bulkext_dir, "pii.txt"), "\t", html)
    ...
    for line in in_file:
        ...
        for match in pronom_links_to_replace:
            new_line = line.replace(
                match,
                '<a href="http://nationalarchives.gov.uk/PRONOM/'
                + match
                + '" target="_blank">'
                + match
                + "</a>",
...
def download_and_cache_html_report_assets(js_directory):
    ...
    try:
        ...
        print(
            "\nERROR: Unable to download JavaSript dependencies. "
...
def main():
    ...
    if not os.path.isdir(js_target):
        shutil.copytree(js_cache, js_target)
